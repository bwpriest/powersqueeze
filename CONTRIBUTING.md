...
`powersqueeze` (`psqz` throughout) is an open source project.
...
`psqz` is distributed under the terms of the
[BSD-Commercial license](https://github.com/LLNL/powersqueeze/blob/master/LICENSE).
...
[issue tracker](https://github.com/LLNL/powersqueeze/issues).
Please first review the existing issues prior to avoid duplicate issues.

If you plan on contributing to `psqz`, please review the
[issue tracker](https://github.com/LLNL/powersqueeze/issues) to check for
...
- Fork the [LLNL powersqueeze repository](https://github.com/LLNL/powersqueeze)
...
- Create a [pull request](https://github.com/LLNL/powersqueeze/compare) from
your branch targeting the most current LLNL `v0.XX-dev` branch

## Forking psqz

If you are not a `psqz` developer at LLNL, you will not have permissions to push
new branches to the repository.
Even `psqz` developers at LLNL will want to use forks for most contributions.
...
If you intend to maintain a persistent fork of `psqz`, it is a best practice to
set the LLNL repository as the `upstream` remote in your fork.
```
$ git clone git@github.com:your_name/powersqueeze.git
$ cd powersqueeze
$ git remote add upstream git@github.com:LLNL/powersqueeze.git
```
...
- A response to a [tracked issue](https://github.com/LLNL/powersqueeze/issues)
(`iss/<issue-number>`)
...
```
Commit code changes to this branch, and add any tests to `${PSQZ_ROOT}/tests`
that validate the correctness of your code, modifying existing tests if need be.
Be sure to add `add_psqz_test(<test-name>)` or `add_psqz_mpi_test(<test-name>)`
to `${PSQZ_ROOT}/tests/CMakeLists.txt` as appropriate.
...
[create a PR](https://github.com/LLNL/powersqueeze/compare).
...
an [issue](https://github.com/LLNL/powersqueeze/issues).
...
Firstly, create a test added to `${PSQZ_ROOT}/tests` that reproduces the bug or
modify an existing test to catch the bug if that is more appropriate.
Be sure to add `add_psqz_test(<test-name>)` or `add_psqz_mpi_test(<test-name>)`
to `${PSQZ_ROOT}/tests/CMakeLists.txt` if you create a new test case.
...
up-to-date and [create a PR](https://github.com/LLNL/powersqueeze/compare).

# Tests

While this is not currently implemented, `psqz` will use GitHub actions for
continuous integration tests.
Our tests will run automatically against every new commit and pull request on
PRs targeting a dev branch, and pull requests must pass all tests prior to being considered for merging into the main project.
...
`psqz`'s tests are contained in the `test` directory, and new tests must be added
manually to `tests/CMakeLists.txt` by adding `add_psqz_test(<test-name>)` or
`add_psqz_mpi_test(<test-name>)` in order to ensure that they are automatically
...
[.clang-format](https://github.com/LLNL/powersqueeze/blob/master/.clang-format)
in the project root.
...
`psqz` also uses [cmake-format](https://github.com/cheshirekow/cmake_format) to
