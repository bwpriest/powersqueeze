...
struct parameters : public detail::parameters {
  parameter<std::uint64_t> vertex_count;
  parameter<std::uint64_t> random_seed;
  parameter<int>           buffer_size;
  parameter<std::string>   mtl_path;
  parameter<bool>          undirected;
  parameter<bool>          use_map;
  parameter<bool>          verbose;

  parameters()
      : detail::parameters(),
        vertex_count("vertex_count", "Number of vertices in the input graph",
                     'c', true, -1),
        random_seed("random_seed", "Random seed", 'z', true, 0),
        buffer_size("buffer_size",
                    "Buffer size used during exponentiation (default \\infty)",
                    'B', true, -1),
        mtl_path("mtl_path", "Metall datastore path (optional)", 'M', true, ""),
        undirected("directed?", "Indicates that the graph stream is undirected",
                   'u', false, false),
        use_map("use_map?", "Indicates ygm::map use (ygm::array otherwise)",
                'm', false, false),
        verbose("verbose?", "Indicates whether to print verbose output", 'V',
                false, false) {
    _params.push_back(&vertex_count);
    _params.push_back(&random_seed);
    _params.push_back(&buffer_size);
    _params.push_back(&mtl_path);
    _params.push_back(&undirected);
    _params.push_back(&use_map);
